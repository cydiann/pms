...
class DynamicApprovalFlowTest(APITestCase):
    ...
    def test_complete_5_level_approval_flow(self):
        ...
        expected_actions = ['submitted', 'approved', 'approved', 'approved', 'approved', 'approved', 'approved']
        self.assertEqual(actions, expected_actions)

        # Check approval levels
        approval_histories = history.filter(action='approved')
        self.assertEqual(approval_histories.count(), 6)  # 4 hierarchy approvals + purchased + delivered
